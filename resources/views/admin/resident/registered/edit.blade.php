            ...
            <div class="col-12">
                <div class="card">
                    <form class="card-body" method="POST"
                        action="{{ route('admin.resident.registered.update', ['resident' => $resident->ulid]) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="d-flex data-field flex-column" id="birth_select">
                            <label for="basic-url" class="form-label fw-bold">Birth ID</label>
                            <input type="text" value="{{ old('resident_birth_id', $resident->birth->id) }}"
                                class="form-control form-control-sm disabled @error('resident_birth_id')
                                        is-invalid
                                    @enderror"
                                id="name" aria-describedby="basic-addon3" disabled>
                        ...
                        <div class="mb-3 data-field d-flex flex-column">
                            ...
                            <div class="input-group">
                                <input type="text" name="email" value="{{ old('email', $resident->email) }}"
                        ...
                        <div class="mb-3 data-field d-flex flex-column">
                            ...
                            <select name="blood_type" class="form-control form-control-sm selectpicker border"
                                ...
                                @foreach (['Islam', 'Protestan', 'Hindu', 'Buddha', 'Khonghucu', 'Khatolik'] as $religion)
                                    <option value="{{ $religion }}"
                                        {{ $resident->religion == $religion ? 'selected' : '' }}>{{ $religion }}
                                    </option>
                                @endforeach
                            </select>
                            @error('religion')
                            ...
                            <select name="blood_type" class="form-control form-control-sm selectpicker border"
                                ...
                                @foreach (['A', 'B', 'AB', 'O'] as $blood_type)
                                    <option value="{{ $blood_type }}"
                                        {{ $resident->blood_type == $blood_type ? 'selected' : '' }}>{{ $blood_type }}
                                    </option>
                                @endforeach
                            </select>
                            @error('blood_type')
                        ...
                        <div class="mb-3 data-field d-flex flex-column">
                            ...
                            <textarea name="address" class="form-control form-control-sm" id="address" title="Blood Type">{{ old('address', $resident->address) }}</textarea>
                            @error('address')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
